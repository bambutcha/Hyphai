    ...
    <section className="flex h-full min-h-0 flex-1 flex-col bg-zinc-900">
      <header className="flex items-center justify-between gap-3 border-b border-emerald-900/30 px-6 py-4">
        <h2 className="text-lg font-medium text-zinc-100">
          {conversationTitle ?? uiText.messages.selectConversation}
        </h2>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 md:px-8">
        {!conversationTitle && (
          <p className="text-center text-sm text-zinc-500">
            {uiText.messages.emptyHint}
          </p>
        )}
        {conversationTitle && loading && <LoadingState label={uiText.messages.loadingMessages} />}
      ...
      <form onSubmit={handleSubmit} className="border-t border-emerald-900/30 p-4 md:p-6">
        <div className="mx-auto flex max-w-3xl gap-3">
          <input
            ...
            placeholder={conversationTitle ? uiText.messages.placeholder : uiText.messages.placeholderNoChat}
